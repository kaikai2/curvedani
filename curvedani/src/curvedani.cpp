...
namespace cAni
{
	...
	iCurveDataSet *iSystem::createCurveDataSet(const char *rendererName, const char *name)
	{
		if (iRenderer *renderer = getRenderer(rendererName))
		{
			return renderer->createCurveDataSet(name);
		}
		return 0;
	}
